...
class CodonOptimizer:
    ...
    def random_reverse_translate(self, protein):
        ...
        dna = "".join(
            [random.choice(reverse_genetic_code[aa]) for aa in protein.upper()]
        )
        return Seq(dna)
    ...
    def change_random_codon(self, seq, start_window, end_window):
        ...
        assert start <= end
        aa_index = random.randint(int(start), int(end))

        nt_index = 3 * aa_index

        aa_at_position = seq[nt_index:nt_index+3].translate()

        random_new_codon = self.random_reverse_translate(aa_at_position)
    ...
    def hillclimb(
        ...
        while True:
            ...
            if new_score > score:
                score = new_score
                assert(new_seq.translate() == seq.translate())
                seq = new_seq
                wait_count = 0
                if verbosity > 0:
