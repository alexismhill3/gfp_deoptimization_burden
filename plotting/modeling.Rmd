...
```{r}
df_prot_cal <- read_csv("calibrate_single_population_charging_1e6.csv")
```
...
df_prot_cal %>% filter(species == "cellularProtein") %>% 
  ...
  scale_x_continuous(name = expression(f[opt]^{cell})) +
...
ggsave("figures/protein_optimal_speed_3.png", width = 7, height = 4)
```

# corresponding python notebook: tacc-grid_search_speed_vs_charging.ipynb

```{r}
df_grid <- read_csv("speed_vs_charging_grid_search.csv")
```
...
ggsave("figures/grid_search.png", width = 12, height = 3)
```

# corresponding python notebook: tacc-ecol-codon-use-trna_skew.ipynb
...
```{r}
df_skew <- read_csv("gfp_regression_ecol_codon_use_speed_0.5_trna_0.7.csv")
```

```{r, fig.width=8, fig.height=2.5}
gfp_plot <- df_skew %>% filter(charging_rate == 100, ecol %in% c(0.6, 0.8, 1.0)) %>%
  ggplot(aes(GFP, cellularProtein_norm, color = factor(codon))) +
    ...
    xlab("GFP") +
    ylab("normalized cell protein") +
    scale_y_continuous(limits = c(0.5, 1)) +
    #scale_x_continuous(breaks= c(2500, 5000, 7500)) +
...
```{r}
# data for each "experiment" w/ different GFP constructs, to be plotted separately
points <- df_skew %>% filter(charging_rate == 100, gfp_rbs_foldx == 30.0, ecol %in% c(0.6, 0.8, 1.0)) %>%
  mutate(tot_protein = GFP + cellularProtein) %>%
  mutate(average_codon_use = ((100 * 1000 * ecol) + (20 * 100 * 30 * codon)) / ((100 * 1000) + (20 * 100 * 30))) %>%
  select(codon, ecol, average_codon_use, cellularProtein, tot_protein)
```


```{r}
base_plot <- function (data) {
  data %>% filter(charging_rate == 100, gfp_rbs_foldx == 30.0) %>%
  mutate(tot_protein = GFP + cellularProtein) %>%
  mutate(average_codon_use = ((100 * 1000 * ecol) + (20 * 100 * 30 * codon)) / ((100 * 1000) + (20 * 100 * 30))) %>%
  ggplot(aes(x=average_codon_use, y=tot_protein)) +
  geom_smooth(se = FALSE, color="black") +
  scale_x_continuous(name = "overall codon use",
...
```{r}
p1_data <- points %>% filter(ecol == 0.6)
p1 <- base_plot(df_skew) + 
...
p2_data <- points %>% filter(ecol == 0.8)
p2 <- base_plot(df_skew) + 
...
p3_data <- points %>% filter(ecol == 1)
p3 <- base_plot(df_skew) + 
...
p1 <- cowplot::plot_grid(gfp_plot + theme(legend.position = "none"), opt_plot, ncol = 1, align = 'v')
cowplot::plot_grid(p1, legend, rel_widths = c(1, 0.2))
ggsave("figures/gfp_experiment_cell_fopt_6_8_10.png", width = 7, height = 4)
```
